...
```{r}
word <- 'tutoring'
raw_data = read.csv("tutoring.csv")
...
```
```{r}
armaorig = arima(dt, order = c(5, 1, 3), seasonal = list(order = c(0, 1, 0), period = 12))
L = 36

armaorig

preds = predict(armaorig, n.ahead = L)
plot(c(dt, preds$pred), type = "l", xlab = "Time", ylab = "Data", main = 'ARMA(5,3)')
...
```
```{r}
armaorig = arima(dt, order = c(5, 1, 3), seasonal = list(order = c(2, 1, 1), period = 12))
L = 36

armaorig

preds = predict(armaorig, n.ahead = L)
plot(c(dt, preds$pred), type = "l", xlab = "Time", ylab = "Data", main = 'ARMA(5,3) X (2,1)_12')
points((n+1):(n+L), data_test, type = "l")
points((n+1):(n+L), preds$pred, type = "l", col = "blue")
```
